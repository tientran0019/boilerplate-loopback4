...
// 1) src/repositories/slugify.repository.base.ts
 ...
 * @deprecated Use the {@link SlugifyRepositoryMixin} instead.
 * eg.
 * ```ts
 * class CustomerRepository extends SlugifyRepositoryMixin<
 *   Customer,
 ...
 * ```
 */
export abstract class DefaultTransactionSlugifyRepository<
...
> extends DefaultTransactionalRepository<E, ID, R> {
	...
	private async generateUniqueSlug(entity: DataObject<E & { slug: string, [key: string]: string | undefined }>): Promise<string> {
		let fields: string[] | string = this.configs?.fields ?? ['name'];
